...
/**
 * Handles exporting order details to PDF format
 */
export class OrderPDFExport {
  /**
   * Generates the HTML content for the order PDF
...
export function OrderPDFGenerator({ order, title }: OrderDetailPDFProps) {
  const handleExport = () => {
    OrderPDFExport.exportToPDF(order, title);
  };

  return {
    exportToPDF: handleExport,
    generatePDFContent: () => OrderPDFExport.generatePDFContent(order, title)
  };
}
